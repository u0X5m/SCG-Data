...
for i in data: 				#Loop over each deck in data
	...
	if i[6] in data_7.keys():
		data_9[i[6]] += set_archetypes  #Count the number of decks for each date
	else:
		data_9[i[6]] = set_archetypes
...
for key, value in data_1.items():
	data_1_lst.append([key, (value/len(data_9[key]))])	#Divide the number of countertop decks by the total number of decks for that period.

for i in data_1_lst:
	j = [(data_2[i[0]])/len(data_9[i[0]]),\
	#For each card, divide the number of decks that contain that card by the number
	#of countertop decks for that date.
	(data_3[i[0]])/len(data_9[i[0]]),\
	(data_4[i[0]])/len(data_9[i[0]]),\
	(data_5[i[0]])/len(data_9[i[0]]),\
	(data_6[i[0]])/len(data_9[i[0]]),\
	(data_7[i[0]])/len(data_9[i[0]]),\
	(data_8[i[0]])/len(data_9[i[0]])]
	i += j
